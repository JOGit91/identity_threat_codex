...
const ATTACKS = [
  ...
  },
  {
    ...
    short: "MFA Fatigue",
    url: "attacks/mfa-fatigue.html",
  },
  {
    id: "mailbox-rule-manipulation",
    name: "Mailbox Rule Manipulation",
    short: "Mailbox Rules",
    url: "attacks/mailbox-rule-manipulation.html",
  },
];
...
const CONTROLS = [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "FIDO2 eliminates push notifications entirely — there is no push to bomb. Phishing-resistant MFA is the single most effective control for MFA fatigue because the attack vector simply does not exist when authenticators don't use push approvals.",
      "mailbox-rule-manipulation": "Mailbox rule manipulation is a post-compromise technique. FIDO2 prevents the phishing-based initial access that typically precedes it — if the attacker can't get in, they can't create rules.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "The primary technical control for MFA fatigue. Number matching requires the victim to type a code shown on the login screen into the Authenticator app — a blind approval no longer works. Additional context (app name + location) makes attacker-triggered pushes immediately suspicious.",
      "mailbox-rule-manipulation": "Number matching stops MFA push bombing as an initial access vector. Fewer successful initial access events means fewer opportunities to reach the mailbox and create rules.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "If an attacker succeeds via MFA fatigue and gains account access, the first persistence move is often registering their own authenticator. This control blocks that post-compromise action.",
      "mailbox-rule-manipulation": "Prevents a key post-compromise persistence move: registering a new MFA method to maintain access even after the legitimate user resets their password. Blocks the attacker from creating a stable foothold from which to continue manipulating mailbox rules.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "Legacy auth protocols bypass MFA entirely — an attacker with the victim's credentials can authenticate via IMAP or POP3 without triggering any push notification. Blocking legacy auth removes this silent alternative to push bombing.",
      "mailbox-rule-manipulation": "Legacy auth protocols (IMAP, EWS with basic auth) can be used to create inbox rules without triggering Conditional Access or MFA. Blocking legacy auth closes this access path to mailbox rule creation.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "Token protection doesn't prevent MFA fatigue from succeeding — the victim's own device completes the authentication. Post-compromise, token binding limits what the attacker can do with any token they acquire.",
      "mailbox-rule-manipulation": "If an attacker gains access via any method, token protection limits their ability to replay that token from a different device. Reduces the window available to create mailbox rules from attacker-controlled infrastructure.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "CAE doesn't prevent MFA fatigue from succeeding — but when the compromise is detected, session revocation takes effect in near-real-time rather than waiting hours for tokens to expire naturally.",
      "mailbox-rule-manipulation": "Fast token revocation via CAE reduces the window an attacker has to create inbox rules after compromise. The sooner access is revoked post-detection, the fewer rules they can establish.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "Sign-in risk policies can detect abnormal MFA activity — repeated failed MFA attempts followed by a successful approval can raise risk level and trigger a block or step-up. Entra ID Protection also surfaces MFA anomaly detections that drive CA enforcement.",
      "mailbox-rule-manipulation": "Sign-in risk CA blocks the high-risk sign-ins that typically precede mailbox rule creation. If a compromised credential is used from an unusual location post-MFA fatigue, risk-based CA can block the session before rules are created.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "Compliant device requirements don't stop MFA fatigue — the attack targets the authentication event, which occurs on the victim's own (potentially compliant) device. Limited direct protection.",
      "mailbox-rule-manipulation": "Requiring a compliant device for Exchange Online access means that even with valid credentials, an attacker on an unmanaged device cannot access the mailbox to create rules.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "Shorter token lifetimes shrink the window after a successful MFA fatigue attack before the attacker is forced to re-authenticate — increasing detection opportunities. Not a prevention control, but limits the blast radius.",
      "mailbox-rule-manipulation": "Reducing session lifetimes means the window available to establish persistent mailbox rules is shorter. An attacker who gains access via any method has less time before re-authentication is required.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "Entra ID Protection detects anomalous MFA activity patterns — repeated MFA denials followed by a suspicious approval, or a successful sign-in from an unfamiliar location immediately following multiple MFA failures.",
      "mailbox-rule-manipulation": "Entra ID Protection's risk signals catch the high-risk sign-in events that typically precede mailbox rule creation. A compromised session used from an unusual location will trigger a risk event that can drive CA block policies.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "MFA fatigue campaigns sometimes begin with an external Teams message impersonating IT or Microsoft support to prime the victim before the push bombing starts. Blocking external Teams access removes this social engineering warm-up vector.",
      "mailbox-rule-manipulation": "External Teams access is not a typical vector for mailbox rule manipulation, which is a post-compromise activity. Not directly relevant.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "Safe Links doesn't directly block MFA push notifications. Some MFA fatigue campaigns include a Teams message with a malicious link as part of the social engineering chain — Safe Links provides protection in that narrow scenario.",
      "mailbox-rule-manipulation": "Safe Links protects against link-based initial access but has no effect on post-compromise mailbox rule creation. Not relevant to this technique.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "MFA fatigue is delivered via push notifications, not file attachments. Not relevant.",
      "mailbox-rule-manipulation": "Mailbox rule manipulation is a server-side post-compromise action. Guest file sharing has no bearing on it.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "MFA fatigue campaigns sometimes culminate in a QuickAssist remote session request after the victim approves an MFA push — removing QuickAssist eliminates this follow-on step even when the initial push bombing succeeds.",
      "mailbox-rule-manipulation": "Mailbox rule manipulation does not involve remote access software. Not relevant.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "Teams app restrictions don't directly prevent MFA push bombing. Not meaningfully relevant to this attack.",
      "mailbox-rule-manipulation": "Mailbox rule manipulation doesn't go through Teams apps. Not relevant.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "Inbox rule creation is a reliable post-MFA-fatigue indicator. After a successful push approval, rule creation is typically the first mailbox action. Alerting on this catches the compromise within minutes.",
      "mailbox-rule-manipulation": "This IS the primary detection control for this attack. Alerting on New-InboxRule with DeleteMessage, ForwardTo, or RedirectTo parameters directly detects the attacker's goal. High-confidence, low-noise signal.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "MDCA detects anomalous session patterns following a suspicious MFA approval — impossible travel, mass download activity, or unusual access patterns that immediately follow an outlier authentication event.",
      "mailbox-rule-manipulation": "MDCA Activity Policies can be configured to alert on suspicious inbox rule patterns — including rules that forward to external domains or delete messages matching specific keywords. Complements the native Unified Audit Log detection.",
    },
    overview: [
...

  {
    ...
    why: {
      ...
      "mfa-fatigue":            "Awareness training is a critical layer for MFA fatigue. Trained employees know: never approve an MFA push you didn't initiate, especially if you're receiving many in a row. Knowing to call IT immediately when this happens — rather than just approving to make it stop — is the behavioral outcome that matters.",
      "mailbox-rule-manipulation": "Mailbox rule manipulation is a post-compromise server-side action that victims typically have no visibility into. Awareness training doesn't directly prevent it, but training users to recognize and report phishing reduces the initial access events that lead to it.",
    },
    overview: [
